    ...
    function get_content() {
        ...
        if(!empty($COURSE)) {
        	...
        	foreach ($facultylist as $faculty) {
        		...
        		$info .= '<a href="' . $CFG->wwwroot . '/user/view.php?id=' .$faculty->id . '&course=' . $COURSE->id . '" title="' . get_string('profileclicktitle','block_faculty_tbird') . $fheader . '">';
	        	...
	        	if(isset($faculty->skype) && $faculty->skype <> '' && !isset($hiddenfields['skypeid'])) {
					...
					$info .= '<strong>' . get_string('skypelabel','block_faculty_tbird') . ':</strong> <a href="skype:'.urlencode($faculty->skype).'?call">'.s($faculty->skype).$statusicon.'</a></br >';
				}
				//finally, add Biography link if set
	        	if(isset($faculty->biourltbird) && $faculty->biourltbird <> '')
					$info .= '<br /><a target="_blank" href="' . urlencode($faculty->biourltbird) . '" title="' . get_string('biotitle','block_faculty_tbird') . '">' . $fheader . get_string('bio','block_faculty_tbird') . '</a></br>';
        	}
        	$facultylist->close();
